function [] = exp_texture_joint_4_2()

exp_num = 4;

% (4) continued, decrease gamma
...
% load nets
net1_init = load('../nets/ivy/all_4_1/des_net', 'net1');
net2_init = load('../nets/ivy/all_4_1/gen_net', 'net2');

% run
num = 2;
total = length(Deltas) * length(Gammas) * length(Ts);
for Delta = Deltas
    for Gamma = Gammas
        for T = Ts
            % log
            ...
            config.Delta = Delta;
            config.Gamma = [1.0*ones(1,100), 0.5*ones(1,100)] * Gamma;
            config.refsig = 1;                                            
            config.T = T;            
            
            % generator net2 parameters
            config.Delta2 = Delta2;
            config.Gamma2 = [1.0*ones(1,100), 0.5*ones(1,100)] * Gamma2;
            config.refsig2 = 1;
            config.s = 0.3;
            config.real_ref = 1;
            config.cap2 = 8;
