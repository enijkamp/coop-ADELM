...
if ismember(1, layer)
    ...
    layer_name = '1';
    num_in = 3;
    num_out = 500; % number of filters in the first layer
    filter_sz = 9;
    %filter_sz = 17; % the square filter with filter size 15x15 in the 1st layer (7)
...
if ismember(2, layer)
    ...
    filter_sz = 7;
    %filter_sz = 11; % the square filter with filter size 15x15 in the 1st layer
    %stride = 3; % sub-sampling size for each filter, 1 means in spatial space, each filter is put every two pixels. 
    stride = 3;
    pad_sz = floor(filter_sz/2); % pad size for image. For example, image is 224x224, and we add two pixels (padded image is 228x228) to handle boundary conditions.
    pad = ones(1,4)*pad_sz;
...
if ismember(3, layer)
    ...
    filter_sz = 6; % the square filter with filter size 15x15 in the 1st layer
